...
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# left_justify_element(element)
# Left justify the finite field element
# and return a signature.
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
def left_justify_element(element):
	for index, number in enumerate(F.ShowCoefficients(element)):
		if number != 0: # or 'if number:'
			#print "inside left_justify_element(element): F.ShowCoefficients(",element,") is", F.ShowCoefficients(element)
...
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# partial_augury(signature, signature)
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# Hand the function a registered chord 
# along with a pitch class collection, 
# and it returns you a list of the 
# "echoes" of that chord in the collection, 
#in register.
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
def partial_augury(signature, pitch_class_collection):
	register_list = []
	...
	prime_form_input_chord = primeform(minweight(left_justify_signature(signature)))
	result = augury(prime_form_input_chord, pitch_class_collection)
	
	for s, x in enumerate(result):
		print "the #",s,"signature in the result is", x
		register_list_working_copy = register_list
		i = 0 
		
		for index, number in enumerate(x):
			if number != 0:
				print "at index", index, "there is number", number, "which we want to replace with", register_list[i]
				# remove indexed value from list
				#L.pop(1) = returns 'green'  
				#L.insert(0, "white")
				x[index] = register_list_working_copy[i]
				i = i + 1
				print x
...
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# agile_primeform(sequence of notes)
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# A function that takes in a sequence of
# notes and returns the prime form that
# corresponds to that 
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# OK. A person walks up to the musical 
# keyboard. She plays a single chord. 
# Organ donor then plays back 4 more chords.
# She then plays a 5-note tune.
# Organ donor plays back the tune slightly 
# differently, using a couple of the chords.

# I do also imagine this as a little like 
# the musical duet section at the end of Close Encounters.

# Also, in keeping with the flash mob, 
# there's a daemon agent who remembers 
# everything that's ever been played and 
# occasionally contaminates the pool with memories.


# So how about this? A function that takes an 
# input sequence of pitches and reduces overlapping 
# segments of it to normal-form chords.
...
def common_tone(u, v):
	...
	for index, number in enumerate(common_notes): 
		# index is the index of (primeform), number is the
		# existence of a pitch class at that index
		#print "at index",index,"there is value",number
		result[number] = 1;
...
print "testing partial_augury(signature, pitch_class_collection)", partial_augury([1,0,0,2,0,0,0,3,0,0,0,0], [1,0,1,0,1,1,0,1,0,1,0,1])

#result = (augury(primeform([1,0,0,1,1,1,0,1,0,0,0,0]), [1,0,1,0,1,1,0,1,0,1,0,1])) == (partial_augury([1,0,0,1,1,1,0,1,0,0,0,0], [1,0,1,0,1,1,0,1,0,1,0,1]))
#print "testing if they return the same thing.", result
...
for i in range(1, (2**12)):
	#lut_dict[F.ShowCoefficients(i)] = primeform(minweight(left_justify_element(i)))
	print >>normal_form_lut_fp, F.ShowCoefficients(i)[1:13]
	print >>interval_vector_lut_fp, "prime form is", primeform(minweight(left_justify_element(i)))
	print >>inversional_index_vector_lut_fp, "prime form is", primeform(minweight(left_justify_element(i)))
	...
	print >>normal_form_lut_fp, primeform(minweight(left_justify_element(i)))
	print >>interval_vector_lut_fp, "interval vector is",intvect(primeform(minweight(left_justify_element(i)))),"\n"
	print >>inversional_index_vector_lut_fp, "inversional index vector is", invindvect(primeform(minweight(left_justify_element(i)))),"\n"
